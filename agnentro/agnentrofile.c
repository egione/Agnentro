...
main(int argc, char *argv[]){
  ...
  do{
    ...
    if((argc!=5)&&(argc!=6)){
      ...
      DEBUG_PRINT("(automask) is 0 to assume that the mask count is (256^(granularity+1)), else 1\nto discover it by examining the file at the given granularity.\n\n");
...
*/
    ...
    if((mode==AGNENTROFILE_MODE_ESTIMATE)||(mode==AGNENTROFILE_MODE_EXACT)||(mode==AGNENTROFILE_MODE_COMPRESS)||(mode==AGNENTROFILE_MODE_DECOMPRESS)){
      if(mode!=AGNENTROFILE_MODE_DECOMPRESS){
        if(mode==AGNENTROFILE_MODE_ESTIMATE){
          agnentroprox_base=agnentroprox_init(0, 0, granularity, loggamma_base, mask_idx_max, mask_max, AGNENTROPROX_MODE_AGNENTROPY, 0, mask_idx_max);
