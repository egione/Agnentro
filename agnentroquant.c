...
main(int argc, char *argv[]){
  ...
  do{
    ...
    if(status|(parameter&(~(AGNENTROQUANT_MODE_CHANNELIZE|AGNENTROQUANT_MODE_DELTAS|AGNENTROQUANT_MODE_NORMALIZE|AGNENTROQUANT_MODE_SATURATE)))){
...
/*
There's one amibiguous case, namely where the fraction bits of mask_u64 are all ones. Rounding up may be wrong, as the uncertainty is 1ULP. However, if we don't round up, then we'll never actually utilize the full range. So just do it. Wrap is impossible because mask_span_in is nonzero.
*/
              mask_u64++;
            }
            if(mask_span_out_power_of_2_status){
